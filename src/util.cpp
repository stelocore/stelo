...
const char * const BITCOIN_CONF_FILENAME = "stelo.conf";
const char * const BITCOIN_PID_FILENAME = "stelod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "stelo" is a composite category enabling all Stelo-related debug output
            if(ptrCategory->count(string("stelo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SteloCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SteloCore
    // Mac: ~/Library/Application Support/SteloCore
    // Unix: ~/.stelocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SteloCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SteloCore";
#else
    // Unix
    return pathRet / ".stelocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty stelo.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override stelo.conf
